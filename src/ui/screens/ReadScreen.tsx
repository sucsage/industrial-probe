...
import { ModbusClient, RegisterResult } from '../../protocols/modbus';

type Phase = 'form' | 'running' | 'done' | 'error';

const FIELDS = ['host', 'port', 'register', 'count', 'unitId'] as const;
...
export function ReadScreen({ onBack }: { onBack: () => void }) {
  ...
  const runRead = async () => {
    ...
    try {
      ...
      const regs = await client.readHoldingRegisters(
        parseInt(values.register) - 40001,
  ...
  return (
    <Box flexDirection="column">
      ...
      {phase === 'running' && <Text color="yellow">Reading registers...</Text>}
