...
export function TestScreen({ onBack }: { onBack: () => void }) {
  ...
  const [summary, setSummary] = useState({ passed: 0, failed: 0, suiteName: '' });
  ...
  const runTests = async () => {
    ...
    client.disconnect();
    setSummary({ passed, failed, suiteName: config.name });
    setPhase('done');
  };

  return (
    <Box flexDirection="column">
      <Header title="Run Test Suite" />
      ...
      {phase === 'form' && (
        <Box flexDirection="column">
          <FormField
            ...
            onSubmit={runTests}
            placeholder="./factory-test.yaml"
          />
          <Box marginTop={1}>
            <Text dimColor>{'Enter to run · ESC back'}</Text>
          </Box>
        </Box>
      )}
